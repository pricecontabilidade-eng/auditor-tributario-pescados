...
class KnowledgeBase:
    ...
    def _load_ncm_rules(self):
        ...
        for line in text.splitlines():
            ...
            m = re.match(
                r"###\s+NCM\s+(\d{8})\s+—\s+(PERMITIDO|VEDADO)",
                line,
            )
            if not m or not current_anexo:
                continue
            ...
            rules.append(
                NcmRule(
                    ncm=ncm,
                    anexo=current_anexo,
                    cclasstrib=cc.group(1).strip() if cc else None,
                    permitido=(flag == "PERMITIDO"),
                    ini=ini.group(1).strip() if ini else None,
                    fim=fim.group(1).strip() if fim else None,
                )
            )
    ...
    def _load_cclass(self):
        ...
        headers = [
            str(x.value).strip() if x.value is not None else ""
            for x in ws[1]
        ]
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            ...
            out[cc] = {
                "cst": (
                    str(g("CST-IBS/CBS")).strip().zfill(3)
                    if g("CST-IBS/CBS") is not None
                    else None
                ),
                "nome": g("Nome cClassTrib"),
                "descricao": g("Descrição cClassTrib"),
                "pred_ibs": g("pRedIBS"),
                "pred_cbs": g("pRedCBS"),
                "ini": g("dIniVig"),
                "fim": g("dFimVig"),
    ...
    def pis_cofins_pescado(self, ncm):
        ...
        com redação dada pela Lei 12.839/2013.

        Esta função não presume tributação normal para produtos
        que estejam fora do benefício.
        """
        n = (ncm or "").replace(".", "").strip().zfill(8)
        ...
        if n == "03029000":
            return {
                ...
                "aliq_cofins": None,
                "fundamento": (
                    "NCM 03029000 excluído da regra específica "
                    "da Lei 12.839/2013"
                ),
            }
        ...
        if n.startswith("0306"):
            return {
                ...
                "aliq_cofins": None,
                "fundamento": (
                    "Crustáceo não abrangido automaticamente pela "
                    "regra específica da Lei 12.839/2013"
                ),
            }
        ...
        if n.startswith("0307"):
            return {
                ...
                "aliq_cofins": None,
                "fundamento": (
                    "Molusco não abrangido automaticamente pela "
                    "regra específica da Lei 12.839/2013"
                ),
            }
